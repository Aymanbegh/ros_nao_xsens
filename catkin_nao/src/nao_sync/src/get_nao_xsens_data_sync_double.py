...
data_xsens_p=list()
xsens_p =list()
xsens_p.append("timestamp")
xsens_p.append("Roll_x")
xsens_p.append("Pitch_y")
xsens_p.append("Yaw_z")
xsens_p.append("omega_x")
xsens_p.append("omega_y")
xsens_p.append("omega_z")
xsens_p.append("alpha_x")
xsens_p.append("alpha_y")
xsens_p.append("alpha_z")
data_xsens_p.append(xsens_p)
...
output_xsens_p = os.path.abspath("/home/begh/DATA/imu_test_p.csv")
...
def gotdata(nao_datas,xsens_datas,xsens_datas_p):
    print("IN")
    #XSENS NAO
    xsens = list()
    line = list()
    ...
    print("got data nao and xsens: {} & {}".format(nao_datas.header.stamp.nsec,time_stamp)) 

    #XSENS Platform
    xsens_p = list()
    orientation_q = xsens_datas_p.orientation
    orientation_list = [orientation_q.x, orientation_q.y, orientation_q.z, orientation_q.w]		    
    (roll,pitch,yaw) = euler_from_quaternion(orientation_list)
    #time_stamp = xsens_datas.header.seq
    a = xsens_datas_p.header.stamp.secs
    b = xsens_datas_p.header.stamp.nsecs
    if(b<100000000 and b>=10000000):
	d = "0"+str(b)
	c = str(a) + str(d)
	#print("Modified1")
    ...
    #c = str(a) + str(b)		  		
    time_stamp = int(c)
    linear_x = xsens_datas_p.linear_acceleration.x
    linear_y = xsens_datas_p.linear_acceleration.y 
    linear_z = xsens_datas_p.linear_acceleration.z
    angular_x = xsens_datas_p.angular_velocity.x 
    angular_y = xsens_datas_p.angular_velocity.y 
    angular_z = xsens_datas_p.angular_velocity.z
    memDatax_p = [time_stamp,roll, pitch,yaw,angular_x ,angular_y ,angular_z ,linear_x, linear_y,linear_z]  
    ...
    for i in range(len(memDatax_p)):
    	xsens_p.append(memDatax_p[i])
    data.append(line)
    data_xsens.append(xsens)
    data_xsens_p.append(xsens_p)
...
xsens_datas_p =message_filters.Subscriber('/imu_00B42E06', Imu)     
...
ats = message_filters.ApproximateTimeSynchronizer([nao_datas, xsens_datas, xsens_datas_p],queue_size=10,slop=0.1)
ats.registerCallback(gotdata)
...
with open(output_xsens_p,"w") as fps_p:
    for xsens_p in data_xsens_p:
	fps_p.write("; ".join(str(y) for y in xsens_p))
	fps_p.write("\n")
